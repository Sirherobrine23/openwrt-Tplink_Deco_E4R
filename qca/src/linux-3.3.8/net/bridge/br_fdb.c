...
int (*guest_fdb_update)(struct net_bridge_port *source, const unsigned char *addr) __rcu __read_mostly;
EXPORT_SYMBOL_GPL(guest_fdb_update);
		   ...
		   const unsigned char *addr)
{
	...
	int (*guest_fdb_check)(struct net_bridge_port *source, const unsigned char *addr);

	guest_fdb_check = rcu_dereference(guest_fdb_update);
	if (guest_fdb_check && guest_fdb_check(source, addr)) {
		return;
	}
