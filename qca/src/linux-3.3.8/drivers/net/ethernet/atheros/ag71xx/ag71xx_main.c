...
void ag71xx_link_adjust(struct ag71xx *ag)
{
	...
	if(pdata->is_qca9561) {
		...
		else if (pdata->is_ar724x)
		{
	    	...
	    	ag71xx_wr(ag, AG71XX_REG_DMA_FIFO_THRESH ,0x01d80160);
		}
		else
			ag71xx_wr(ag, AG71XX_REG_FIFO_CFG3, 0x008001ff);

		if(ag->gmac_num == 0 && pdata->is_ar724x) {
			if (!ag->duplex){
				/* half duplex */
				ag71xx_wr(ag, AG71XX_REG_MAC_CFG1, MAC_CFG1_INIT & (~MAC_CFG1_TFC));
				ag71xx_wr(ag, AG71XX_REG_FIFO_CFG3, 0x00f00040);
				ag71xx_wr(ag, AG71XX_REG_DMA_FIFO_THRESH, 0x00880060);
			}else{
				/* full duplex */
				ag71xx_wr(ag, AG71XX_REG_MAC_CFG1, MAC_CFG1_INIT & (~MAC_CFG1_RFC));
			}
		}

		if(ag->gmac_num == 1 && pdata->is_ar724x) {
			ag71xx_wr(ag, AG71XX_REG_MAC_CFG1, MAC_CFG1_INIT & (~MAC_CFG1_TFC));
		}
	} else {
		if (pdata->is_ar91xx)
...
int32_t (*sfe_PppoeToWanParsePtr)(struct sk_buff * skb) = NULL;
EXPORT_SYMBOL(sfe_PppoeToWanParsePtr);
int32_t (*sfe_PptpToWanParsePtr)(struct sk_buff * skb) = NULL;
EXPORT_SYMBOL(sfe_PptpToWanParsePtr);
int32_t (*sfe_L2tpToWanParsePtr)(struct sk_buff * skb) = NULL;
EXPORT_SYMBOL(sfe_L2tpToWanParsePtr);
#endif
			     ...
			     bool is_ar7240)
{
	...
	/* add by wanghao  */
#ifdef CONFIG_SFE_PPP_HEADER
	if (sfe_PppoeToWanParsePtr)
	{
		sfe_PppoeToWanParsePtr(skb);
	}
	
	if (sfe_PptpToWanParsePtr)
	{
		sfe_PptpToWanParsePtr(skb);
	}

	if (sfe_L2tpToWanParsePtr)
	{
		sfe_L2tpToWanParsePtr(skb);
	}
#endif
	/* add end  */
...
//#define CONFIG_ACTIVE_FLOW_CONTROL
/* add by wanghao  */
...
#define DISCARD_CONTINUOUSLY
//#define DISCARD_INTERVAL

#define INCREASE_DROP_COUNT			6
#define DECREASE_DROP_COUNT			20
#define DROP_COUNT_STEP				2
#define MAX_DROP_COUNT				6//10
#define RXRING_USAGE					98//20
#define MIN_DROP_INTERVAL				4
#define MAX_DROP_INTERVAL				16
#define DATA_PACKET_LEN				524
#define RXRING_COUNT_TIME				msecs_to_jiffies(200)

static int pktCountTest = 0;
static int pktCountTestIn = 0;
int dropPercent = 15;
EXPORT_SYMBOL(dropPercent);

static unsigned int dropLock = 1;
static unsigned int rxRingfullCount = 0;
static unsigned int rxRingVacantCount = 0;
static int dropCount = 0;
//static int dropIntervalCount = 0;
static int dropInterval = MAX_DROP_INTERVAL;

extern struct task_struct *pppdTask;
...
static unsigned long rxRingCountTime = 0;

extern int get_ring_usage(int mode, int i);
...
static int ag71xx_rx_packets(struct ag71xx *ag, struct net_device *dev, int limit)
{
	...
	has_ar8216 = ag71xx_has_ar8216(ag);
		...
		unsigned int dropIntervalCount = 0;

	if (forceDropFlag == 1)
	{
	//#ifdef DISCARD_CONTINUOUSLY
		dropCount = MAX_DROP_COUNT;
		if (dropLock == 1)
		{
			printWar("Alert, force drop!dropCount increase to %d\n", dropCount);
		}
    /*
	#else ifdef DISCARD_INTERVAL
		dropInterval = MIN_DROP_INTERVAL;
		if (dropLock == 1)
		{
			printWar("Alert, force drop!dropInterval decrease to %d\n", dropInterval);
		}
	#endif
    */
	...
	{
		if (time_after(jiffies, rxRingCountTime))
		{
			rxRingVacantCount++;
			rxRingCountTime = jiffies + RXRING_COUNT_TIME;
		}
		if (rxRingVacantCount >= DECREASE_DROP_COUNT)
		{
			dropLock = 1;
			rxRingVacantCount = 0;
        /*
			...
			if (dropCount > 0)
			{
				dropCount -= DROP_COUNT_STEP;
				printWar("dropCount decrease to %d\n", dropCount);
			}
		#else ifdef DISCARD_INTERVAL
			//dropIntervalCount = 0;
			if (dropInterval < MAX_DROP_INTERVAL)
			{
				dropInterval += DROP_COUNT_STEP;
				printWar("dropInterval increase to %d\n", dropInterval);
			}
		#endif
        */
		}
...
/* add by wanghao for debug  */
	...
	/*if (1)
	{
		//drop every pkt for debug
	}*/
	#if 0
	if ((skb->data[24] == 0xc0 && skb->data[25] == 0x21) || (skb->data[28] == 0xc0 && skb->data[29] == 
	0x21))
	{
		skb_debugFy(skb);
		printWar("limit = %d\n",limit);
		...
		#if 0
		if (pppdTask && pppdTask->state != TASK_RUNNING)
		{
			//printErr("wake up pppd!\n");
			wake_up_process(pppdTask);
		}
		#endif
	}
	#endif
	if (dropLock == 0)
	{
#if 0
	#ifdef DISCARD_CONTINUOUSLY
		if (skb->len > DATA_PACKET_LEN && pktCount < dropCount)
	#else ifdef DISCARD_INTERVAL
		if (skb->len > DATA_PACKET_LEN && dropIntervalCount++ > dropInterval)
	#endif
		{
			if (printk_ratelimit())
			{
				printErr("Drop packet due to shity CPU resource!\n");
			}
			dropIntervalCount = 0;
			pktCount++;
		...
		/*else if (pktCount >= dropCount)
		{
			printErr("drop %d pkts!\n", pktCount);
		}*/
#endif
        if(pktCountTest++ > 100)
		{
		...
		if (skb->len > DATA_PACKET_LEN)
		{
			if(pktCountTestIn < dropPercent) 
			{
			#if 1
				if (printk_ratelimit())
				{
					printErr("Drop packet due to shity CPU resource!  dropPercent= %d,pktCountTest = %d\n",dropPercent,pktCountTest);
				}
			#endif
				pktCountTestIn ++;
				dev->stats.rx_dropped++;
...
/* add end */
    ...
    if(skb_data[4] == 0xb7 && skb_data[5] == 0xf8 && skb_data[10] == 0xbb && skb_data[11] == 0x08) {
        printk("\r\n");
        for(idx = 0; idx <64; idx++) {
            printk("%02X-", skb_data[idx]);
        }
        printk("\r\n");
    }
    skb->port_num = 8;
		...
		if (*(uint16_t *)&skb->data[athr_tag_offset] == athr_tag) {
			...
			skb->port_num = ((aos_header_t *)skb->head)->sport;
