...
extern unsigned short guest_vid;
#endif
static int ar8216_get_vlan_dev(struct phy_device *phydev, int idx)
{
    struct ar8216_priv *priv = phydev->priv;
    uint8_t  name[IFNAMSIZ] = {0};
    u16 vlanID = priv->vlan_id[idx];

    snprintf(name, sizeof(name) - 1, "%s.%d\0", phydev->attached_dev->name, vlanID);

    priv->vlan_dev[idx] = dev_get_by_name(&init_net, name);
    if(priv->vlan_dev[idx] != NULL){
        dev_put(priv->vlan_dev[idx]);
        return 1;
    }
...
ar8216_read_status(struct phy_device *phydev)
{
    ...
    if (priv->chip_ver == AR8XXX_VER_AR8337)
    {
        ...
        if (change) {
            printk("%s:dev[%s] oldstatus[%d] status[%d]\r\n", __func__, dev->devname, priv->old_port_status, port_status);
        }

        for(i = 1; i < AR8X16_MAX_VLANS; i++) {

            if(!priv->vlan_table[i])
                break;

            if(!ar8216_get_vlan_dev(phydev, i))
                continue;
            ...
            if((port_status & priv->vlan_table[i]) != 0) {
                ...
                if(!netif_carrier_ok(priv->vlan_dev[i])) {
                        ...
                        printk("%s-%d: start netif_carrier_on for dev[%s] vid[%x] i[%d] vlan_table[%d]\r\n", 
                                __func__, __LINE__, priv->vlan_dev[i]->name, priv->vlan_id[i], i, priv->vlan_table[i]);
                ...
                if(0
                    ...
                    || (priv->vlan_dev[i]->priv_flags & IFF_HNAT_WIFI_REDIRECT_PORT)
#endif
#ifdef CONFIG_GUEST_SGMAC
                    || (priv->vlan_id[i] == guest_vid)
#endif
                ) {
                        
                        if(!netif_carrier_ok(priv->vlan_dev[i])) {
                            printk("%s_%d: %s carrier on[HNAT_WIFI_REDIRECT_PORT or guest if]!\n", __FUNCTION__, __LINE__, priv->vlan_dev[i]->name);
                            netif_carrier_on(priv->vlan_dev[i]);
                        }
                }
                else
                if(netif_carrier_ok(priv->vlan_dev[i])) {
                        ...
                        printk("%s-%d: start netif_carrier_off for dev[%s] vid[%x] i[%d] vlan_table[%d]\r\n", 
                                __func__, __LINE__, priv->vlan_dev[i]->name, priv->vlan_id[i], i, priv->vlan_table[i]);
